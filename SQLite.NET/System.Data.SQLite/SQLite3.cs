...
namespace System.Data.SQLite
{
  ...
  internal class SQLite3 : SQLiteBase
  {
...
#if !PLATFORM_COMPACTFRAMEWORK
    internal const string DesignerVersion = "1.0.111.0";
#endif
    ...
    internal static string SQLiteVersion
    {
      get
      {
        return PtrToString(UnsafeNativeMethods.sqlite3_libversion(), -1);
      }
    }
    ...
    internal static string SQLiteSourceId
    {
      get
      {
        return PtrToString(UnsafeNativeMethods.sqlite3_sourceid(), -1);
      }
    }
    ...
    internal static string SQLiteCompileOptions
    {
        get
        {
            ...
            while (zValue != IntPtr.Zero)
            {
                ...
                result.Append(PtrToString(zValue, -1));
    ...
    internal static string InteropVersion
    {
        ...
        {
#if !SQLITE_STANDARD
            return PtrToString(UnsafeNativeMethods.interop_libversion(), -1);
#else
            return null;
#endif
        }
    ...
    internal static string InteropSourceId
    {
        ...
        {
#if !SQLITE_STANDARD
            return PtrToString(UnsafeNativeMethods.interop_sourceid(), -1);
#else
            return null;
#endif
        }
    ...
    internal override string GetFileName(string dbName)
    {
        ...
        return PtrToString(UnsafeNativeMethods.sqlite3_db_filename_bytes(
            _sql, ToBytes(dbName)), -1);
    }
    ...
    internal override void Open(string strFilename, string vfsName, SQLiteConnectionFlags connectionFlags, SQLiteOpenFlagsEnum openFlags, int maxPoolSize, bool usePool)
    {
      ...
      if (_sql == null)
      {
        ...
        finally /* NOTE: Thread.Abort() protection. */
        {
          ...
          if (extFuncs != 0)
          {
            n = UnsafeNativeMethods.sqlite3_open_interop(ToBytes(strFilename), ToBytes(vfsName), openFlags, extFuncs, ref db);
          }
          else
#endif
          {
            n = UnsafeNativeMethods.sqlite3_open_v2(ToBytes(strFilename), ref db, openFlags, ToBytes(vfsName));
          }
...
#if !SQLITE_STANDARD
        int len = 0;
        name = PtrToString(UnsafeNativeMethods.sqlite3_bind_parameter_name_interop(handle, index, ref len), len);
#else
    ...
    {
        SQLiteStatementHandle handle = stmt._sqlite_stmt;
        int index = UnsafeNativeMethods.sqlite3_bind_parameter_index(handle, ToBytes(paramName));
    ...
    internal override string ColumnType(SQLiteStatement stmt, int index, ref TypeAffinity nAffinity)
    {
        ...
        {
            string declType = PtrToString(p, len);
...
#if !SQLITE_STANDARD
      int len = 0;
      return PtrToString(UnsafeNativeMethods.sqlite3_column_origin_name_interop(stmt._sqlite_stmt, index, ref len), len);
#else
...
#if !SQLITE_STANDARD
      int len = 0;
      return PtrToString(UnsafeNativeMethods.sqlite3_column_database_name_interop(stmt._sqlite_stmt, index, ref len), len);
#else
...
#if !SQLITE_STANDARD
      int len = 0;
      return PtrToString(UnsafeNativeMethods.sqlite3_column_table_name_interop(stmt._sqlite_stmt, index, ref len), len);
#else
    ...
    internal override bool ColumnMetaData(string dataBase, string table, string column, bool canThrow, ref string dataType, ref string collateSequence, ref bool notNull, ref bool primaryKey, ref bool autoIncrement)
    {
      ...
      dtLen = 0;
      csLen = 0;
      n = UnsafeNativeMethods.sqlite3_table_column_metadata_interop(_sql, ToBytes(dataBase), ToBytes(table), ToBytes(column), ref dataTypePtr, ref collSeqPtr, ref nnotNull, ref nprimaryKey, ref nautoInc, ref dtLen, ref csLen);
#else
      dtLen = -1;
      csLen = -1;
      ...
      dataType = PtrToString(dataTypePtr, dtLen);
      collateSequence = PtrToString(collSeqPtr, csLen);
...
#if !SQLITE_STANDARD
      int len = 0;
      return PtrToString(UnsafeNativeMethods.sqlite3_column_text_interop(stmt._sqlite_stmt, index, ref len), len);
#else
...
#if !SQLITE_STANDARD
      n = UnsafeNativeMethods.sqlite3_create_function_interop(_sql, ToBytes(strFunction), nArgs, 4, IntPtr.Zero, func, funcstep, funcfinal, (needCollSeq == true) ? 1 : 0);
      if (n == SQLiteErrorCode.Ok) n = UnsafeNativeMethods.sqlite3_create_function_interop(_sql, ToBytes(strFunction), nArgs, 1, IntPtr.Zero, func, funcstep, funcfinal, (needCollSeq == true) ? 1 : 0);
#else
    ...
    {
      SQLiteErrorCode n = UnsafeNativeMethods.sqlite3_create_collation(_sql, ToBytes(strCollation), 2, IntPtr.Zero, func16);
      if (n == SQLiteErrorCode.Ok) n = UnsafeNativeMethods.sqlite3_create_collation(_sql, ToBytes(strCollation), 1, IntPtr.Zero, func);
      if (canThrow && (n != SQLiteErrorCode.Ok)) throw new SQLiteException(n, GetLastError());
      return n;
    }
    ...
    internal override CollationSequence GetCollationSequence(SQLiteFunction func, IntPtr context)
    {
      ...
      if (p != null) seq.Name = PtrToString(p, len);
...
#if !SQLITE_STANDARD
      int len = 0;
      return PtrToString(UnsafeNativeMethods.sqlite3_value_text_interop(ptr, ref len), len);
#else
    ...
    {
      UnsafeNativeMethods.sqlite3_result_error(context, ToBytes(value), value.Length);
    }
    ...
    {
      byte[] b = ToBytes(value);
      UnsafeNativeMethods.sqlite3_result_text(context, ToBytes(value), b.Length - 1, (IntPtr)(-1));
    }
    ...
    internal override void LoadExtension(string fileName, string procName)
    {
        ...
        try
        {
            ...
            if (n != SQLiteErrorCode.Ok)
                throw new SQLiteException(n, PtrToString(pError, -1));
        }
        finally
        {
    ...
    {
      UnsafeNativeMethods.sqlite3_log(iErrCode, ToBytes(zMessage));
    }
        ...
        )
    {
        ...
        byte[] zDestName = ToBytes(destName);
        byte[] zSourceName = ToBytes(sourceName);
    ...
    internal override void GetIndexColumnExtendedInfo(string database, string index, string column, ref int sortMode, ref int onError, ref string collationSequence)
    {
      ...
      rc = UnsafeNativeMethods.sqlite3_index_column_info_interop(_sql, ToBytes(database), ToBytes(index), ToBytes(column), ref sortMode, ref onError, ref coll, ref colllen);
      if (rc != SQLiteErrorCode.Ok) throw new SQLiteException(rc, null);

      collationSequence = PtrToString(coll, colllen);
#else
      sortMode = 0;
      onError = 2;
    ...
    {
      return UnsafeNativeMethods.sqlite3_file_control(_sql, (zDbName != null) ? ToBytes(zDbName) : null, op, pArg);
    }
  }
}
